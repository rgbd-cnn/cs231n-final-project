...
def run_inception_resnet_test(data, num_classes, device, recover, ckpt_path,
  ...
  model = setup_resnet_inception_model(data_shape, num_classes, 1, 2, 1,
  ...
  if tensorboard_log_dir:
    train_dir = os.path.join(os.path.expanduser(tensorboard_log_dir),
                             "IR-%s-lr-%s-reg-%s-train" % (dataset, lr, reg))
    val_dir = os.path.join(os.path.expanduser(tensorboard_log_dir),
                           "IR-%s-lr-%s-reg-%s-val" % (dataset, lr, reg))
  ...
  for i in range(num_train_val_cycles):
    ...
    train_model(device, sess, model, data['X_train'], data['y_train'],
                epochs=train_epochs_per_validation, batch_size=64,
                is_training=True, log_freq=100, plot_loss=False,
                global_step=global_step, writer=train_writer)
    ...
    if tensorboard_log_dir:
      ...
      train_model(device, sess, model, data['X_val'], data['y_val'], epochs=1,
                  ...
                  plot_loss=False, global_step=global_step, writer=val_writer)
