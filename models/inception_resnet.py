...
def inception_res_C(input, counter, is_training):
    with tf.variable_scope("inc_res_C" + str(counter)):
        # Batch Normalization
        out = slim.batch_norm(input, decay=0.99, center=True, scale=True,
                              epsilon=1e-8,
        ...
        out_2 = slim.batch_norm(out_2, decay=0.99, center=True, scale=True,
                                epsilon=1e-8,
                                activation_fn=None, is_training=is_training,
                                trainable=True)
        ...
        out = slim.batch_norm(out, decay=0.99, center=True, scale=True,
                              epsilon=1e-8,
                              activation_fn=None, is_training=is_training,
                              trainable=True)
        out = tf.nn.relu(out)
...
def reduction_B(input, is_training):
    with tf.variable_scope("reduction_B"):
        ...
        out_2 = slim.batch_norm(out_2, decay=0.99, center=True, scale=True,
                                ...
                                trainable=True)
        out_2 = tf.nn.relu(out_2)
        out_2 = slim.conv2d(out_2, 256, [3, 3], stride=2, padding='VALID',
                            activation_fn=None)

        # Inception Path 3
        out_3 = slim.conv2d(out_relu, 128, [1, 1], activation_fn=None)
        out_3 = slim.batch_norm(out_3, decay=0.99, center=True, scale=True,
                                epsilon=1e-8,
                                activation_fn=None, is_training=is_training,
                                trainable=True)
        out_3 = tf.nn.relu(out_3)
        out_3 = slim.conv2d(out_3, 256, [3, 3], stride=2, padding='VALID',
                            activation_fn=None)

        # Inception Path 4
        out_4 = slim.conv2d(out_relu, 128, [1, 1], activation_fn=None)
        out_4 = slim.batch_norm(out_4, decay=0.99, center=True, scale=True,
                                epsilon=1e-8,
                                activation_fn=None, is_training=is_training,
                                trainable=True)
        ...
        out_4 = slim.batch_norm(out_4, decay=0.99, center=True, scale=True,
                                ...
                                trainable=True)
        out_4 = tf.nn.relu(out_4)
        out_4 = slim.conv2d(out_4, 256, [3, 3], stride=2, padding='VALID',
                            activation_fn=None)
...
def reduction_A(input, is_training):
    with tf.variable_scope("reduction_A"):
        ...
        # Inception Path 2
        out_2 = slim.conv2d(out_relu, 128, [3, 3], stride=2, padding='VALID',
                            activation_fn=None)
...
def inception_res_model(input, num_A, num_B, num_C, num_classes, is_training):
    ...
    output = slim.fully_connected(slim.layers.flatten(out), num_classes,
                                  activation_fn=None)
