...
def setup_depth_enhanced_cnn_model(image_size, num_classes, A, B, C,
    ...
    model['depth_map'] = depth_map_normalized
    model['y_out'] = y_out
    model['loss_val'] = loss
    model['train_step'] = train_step
    model['net'] = net
    model['embedding'] = embedding
    model['embedding_train'] = tf.Variable(tf.random_normal([num_train, 2560]),
                                           name="final_embedding_train")
    model['embedding_val'] = tf.Variable(tf.random_normal([num_val, 2560]),
